  ...
  {:else}
    <div class="teams-layout">
      ...
      <section class="team-detail" aria-labelledby="team-name">
        <header class="team-heading"><div><h2 id="team-name">{selectedTeam.name}</h2><p>{members.length} {members.length === 1 ? 'membre' : 'membres'}</p></div><div class="team-actions"><button class="icon-text-button" type="button" on:click={copyEmails} disabled={members.length === 0} title="Copier les emails"><svelte:component this={copied ? Check : Clipboard} size={18} aria-hidden="true" />{copied ? 'Copiés' : 'Copier les emails'}</button><a class="primary-button" href={`#personnes/nouveau?equipe=${selectedTeam.id}`}><Plus size={18} aria-hidden="true" /> Ajouter</a></div></header>
